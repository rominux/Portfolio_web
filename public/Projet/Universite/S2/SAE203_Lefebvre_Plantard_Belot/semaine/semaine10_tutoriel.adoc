...
Avant de commencer à versionner notre code, nous avons dû configurer notre identité sur notre machine pour que nos futurs <<glossaire-commit, commits>> nous soient bien attribués :
[source,bash]
----
...
git log
----
.Historique des 3 premiers commits avec git log
image::img/git-log-3commits.png[Résultat du git log, pdfwidth=100%, align="center"]

===  Comparaison de versions avec git diff

Nous avons utilisé `git diff` pour comparer deux commits. Voici la sortie obtenue :
[source,diff]
----
...
===  Gestion des branches et fusions (<<glossaire-merge, Merge>>)
...
.Création, bascule et fusion d'une branche sous Git
...
>>>>>>> conflit
----
.Affichage d'un conflit de fusion dans l'éditeur nano
...
Pour travailler en équipe, nous avons utilisé un dépôt distant sur GitLab. Nous avons d'abord généré une clé <<glossaire-ssh, SSH>> (ssh-keygen -t ed25519 -C "exemple@univ-lille.fr"`) que nous avons ajoutée à notre profil GitLab pour l'authentification.
...
.Traçabilité des lignes de code via la commande git blame
...
Bien que la ligne de commande soit rapide, nous avons testé une interface graphique web nommée **Ungit**. Après avoir installé <<glossaire-nodejs, Node.js>> et <<glossaire-npm, npm>> sur le système, nous l'avons installé via la commande globale : `sudo -H npm install -g ungit`. Ensuite, il se lance en tapant `ungit` et s'ouvre directement sous forme d'onglet dans le navigateur web.

.Interface web de l'outil graphique Ungit
...
.Visualisation des différences de code (diff) dans Ungit
...
* **La panique du conflit de fusion :** Lors de notre premier conflit, voir le fichier rempli de chevrons `<<<<<<<` a été perturbant. Nous avons d'abord cru avoir cassé le code. Comprendre qu'il s'agissait simplement de texte à nettoyer manuellement dans un éditeur nous a rassurés .
